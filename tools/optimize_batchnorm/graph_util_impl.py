...
def remove_training_nodes(input_graph, protected_nodes=None):
  ...
  for node in input_nodes:
    ...
    #if node.op in types_to_remove and 'Postprocessor' not in node.name:
  ...
  for node in nodes_after_removal:
    ...
    if node.op in types_to_splice and node.name not in protected_nodes and 'Postprocessor' not in node.name\
            and 'Preprocessor' not in node.name:
