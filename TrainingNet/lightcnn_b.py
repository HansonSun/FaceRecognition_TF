...
def inference(image,dropout_keep_prob=0.8,phase_train=True,scope="inference",weight_decay=0.0,bottleneck_layer_size=256):
	end_poins={}
	with tf.variable_scope(scope, 'inference', [image]):
		with slim.arg_scope([slim.conv2d, slim.fully_connected],activation_fn=None,
			...
			#droupout
			net = slim.dropout(net, dropout_keep_prob, is_training=phase_train,scope='Dropout')
