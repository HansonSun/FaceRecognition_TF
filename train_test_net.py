...
def Train_Test_Net(images, phase):

    if phase==solver.train:
        image_batch_list=[]
        for i in range(solver.batch_size):
            #random flip
            tmp_img=tf.image.random_flip_left_right(images[i])
            #random crop
            tmp_img=tf.random_crop(tmp_img[...,0],size=(solver.train_crop_width,solver.train_crop_height ) )
            tmp_img=tf.reshape(tmp_img,(solver.train_crop_width,solver.train_crop_height ,1))
            image_batch_list.append(tmp_img)

        x_image = tf.stack(image_batch_list, 0 ) #x_image shape :[?:128:128:1]
    ...
    #Drop1
    drop1 = tf.nn.dropout(eltwise5, keep_prob)

    #FC2
    W_fc2=tf.get_variable( name="w_fc2",shape=[256, solver.CLASS_NUM],initializer=tf.contrib.layers.xavier_initializer() )
    b_fc2=tf.get_variable( name="b_fc2",initializer=tf.constant(0.1,shape=[solver.CLASS_NUM]) )
