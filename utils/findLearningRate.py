...
def run_training():
    ...
    iterator,next_element = input_data.input_images_data( config.finetune_dataset_path,config.batch_size)
    ...
    variable_saver=tf.train.Saver(slim.get_variables(scope="InceptionResnetV1"))
    ...
    with tf.Session() as sess:
        variable_saver.restore( sess,tf.train.latest_checkpoint(config.finetune_model_dir) )
